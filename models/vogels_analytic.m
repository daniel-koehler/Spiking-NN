...
% without R_M
%Y(:,1) = exp(-t/T_M)*(V_Rest*exp(t/T_M) + (T_M^2*Y_0(:,1) - T_M^2*V_Rest + T_E*T_I*Y_0(:,1) - T_E*T_M*Y_0(:,1) - T_E*T_I*V_Rest - T_I*T_M*Y_0(:,1) + T_E*T_M*V_Rest + T_I*T_M*V_Rest - E_E*T_E*T_I*Y_0(:,2) + E_E*T_E*T_M*Y_0(:,2) - E_I*T_E*T_I*Y_0(:,3) + E_I*T_I*T_M*Y_0(:,3) + T_E*T_I*V_Rest*Y_0(:,2) + T_E*T_I*V_Rest*Y_0(:,3) - T_E*T_M*V_Rest*Y_0(:,2) - T_I*T_M*V_Rest*Y_0(:,3))/((T_E - T_M)*(T_I - T_M))) + (T_E*Y_0(:,2)*exp(-t/T_E)*(E_E - V_Rest))/(T_E - T_M) + (T_I*Y_0(:,3)*exp(-t/T_I)*(E_I - V_Rest))/(T_I - T_M);
% with R_M
%Y(:,1) = exp(-t/T_M)*(V_Rest*exp(t/T_M) + (T_M^2*Y_0(:,1) - T_M^2*V_Rest + T_E*T_I*Y_0(:,1) - T_E*T_M*Y_0(:,1) - T_E*T_I*V_Rest - T_I*T_M*Y_0(:,1) + T_E*T_M*V_Rest + T_I*T_M*V_Rest - E_E*R_M*T_E*T_I*Y_0(:,2) + E_E*R_M*T_E*T_M*Y_0(:,2) - E_I*R_M*T_E*T_I*Y_0(:,3) + E_I*R_M*T_I*T_M*Y_0(:,3) + R_M*T_E*T_I*V_Rest*Y_0(:,2) + R_M*T_E*T_I*V_Rest*Y_0(:,3) - R_M*T_E*T_M*V_Rest*Y_0(:,2) - R_M*T_I*T_M*V_Rest*Y_0(:,3))/((T_E - T_M)*(T_I - T_M))) - (T_E*Y_0(:,2)*exp(-t/T_E)*(R_M*V_Rest - E_E*R_M))/(T_E - T_M) - (T_I*Y_0(:,3)*exp(-t/T_I)*(R_M*V_Rest - E_I*R_M))/(T_I - T_M);
Y(:,2) = Y_0(:,2)*exp(-t/T_E);
Y(:,3) = Y_0(:,3)*exp(-t/T_I);